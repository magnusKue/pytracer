...
class Parser:
    def __init__(self, defaultSamples, defaultImageWidth, defaultImageHeight, defaultBounceCap, defaultDest, defaultPreview, defaultWhite) -> None:
        self.parser = argparse.ArgumentParser(description=f'{Back.GREEN}{Fore.BLACK} - A simple python raytracer - {Style.RESET_ALL}')

        self.parser.add_argument('--scene', "-S", "-scn",        type=str,      help=f"{Fore.GREEN}Load a scene from world.json. {Fore.RED}No defaults, needs to be specified{Style.RESET_ALL}")
        ...
        self.parser.add_argument('--resolution', "-r", "-res",   type=str,      default=None,             help=f'{Fore.GREEN}Image resolution as WIDTHxHEIGHT (Bsp: "600x400"). {Fore.RED}Default is {defaultImageWidth}x{defaultImageWidth}{Style.RESET_ALL}')
    ...
    def getArgs(self):
        ...
        self.resolution = args.resolution.split("x")
        if len(self.resolution) != 2:
            Error.raiseError(
                type="invalid resolution",
                info="please enter a valid resolution. \nExample: 200x300",
                quitSrc=True
            )
